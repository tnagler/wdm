       ...
       std::vector<double> weights = std::vector<double>())
{
  ...
  std::vector<double> R_X = rank0(x, weights);
  std::vector<double> R_Y = rank0(y, weights);
  std::vector<double> S_X, S_Y;
  if (weights.size() > 0) {
    S_X = rank0(x, utils::pow(weights, 2));
    S_Y = rank0(y, utils::pow(weights, 2));
  } else {
    S_X = R_X;
    S_Y = R_Y;
  }
  ...
  for (size_t i = 0; i < x.size(); i++) {
    ...
    A_2 += ((R_X[i] * R_Y[i] - S_XY[i]) * R_XY[i] -
            S_XY[i] * (R_X[i] + R_Y[i]) + 2 * T_XY[i]) *
           weights[i];
    A_3 += ((R_X[i] * R_X[i] - S_X[i]) * (R_Y[i] * R_Y[i] - S_Y[i]) -
...
phoeffb(double B, double n)
{
  ...
  if ((B <= 1.1) | (B >= 8.5)) {
    ...
    std::vector<double> grid{
      ...
      4.95, 5,    5.5,  6,    6.5,  7,    7.5,  8,    8.5
    };
    std::vector<double> vals{
      0.5297, 0.4918, 0.4565, 0.4236, 0.3930, 0.3648, 0.3387, 0.3146,  0.2924,
      0.2719, 0.2530, 0.2355, 0.2194, 0.2045, 0.1908, 0.1781, 0.1663,  0.1554,
      0.1453, 0.1359, 0.1273, 0.1192, 0.1117, 0.1047, 0.0982, 0.0921,  0.0864,
      0.0812, 0.0762, 0.0716, 0.0673, 0.0633, 0.0595, 0.0560, 0.0527,  0.0496,
      0.0467, 0.0440, 0.0414, 0.0390, 0.0368, 0.0347, 0.0327, 0.0308,  0.0291,
      0.0274, 0.0259, 0.0244, 0.0230, 0.0217, 0.0205, 0.0194, 0.0183,  0.0173,
      0.0163, 0.0154, 0.0145, 0.0137, 0.0130, 0.0123, 0.0116, 0.0110,  0.0104,
      0.0098, 0.0093, 0.0087, 0.0083, 0.0078, 0.0074, 0.0070, 0.0066,  0.0063,
      0.0059, 0.0056, 0.0053, 0.0050, 0.0047, 0.0045, 0.0042, 0.00025, 0.00014,
