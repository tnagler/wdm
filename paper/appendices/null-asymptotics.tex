\section{Kendall calibration}
\label{app:null-asymptotics}

This appendix records the algebra behind the Kendall calibration described in
Section~\ref{subsec:standard-tests}. Three statements are involved. Without
ties, the weighted concordance numerator has an exact conditional null
variance. With ties and uniform masses, the variance conditional on the two
observed marginal tie patterns is an exact permutation identity. For unequal
masses, the software inserts fractional pseudo-counts into that identity; the
resulting construction has the correct first-order null limit under diffuse
exogenous weighting, but is not an exact finite-sample permutation variance.

\subsection{Exact conditional variance without ties}

With $S_w$ as in Section~\ref{sec:standard-measures} and no ties, the
weighted numerator has exact conditional null variance
\begin{align*}
...
\end{align*}
because each pair kernel has variance one, disjoint pair kernels are
independent, and two kernels that share one observation have covariance
$1/9$. Since $\widehat\tau_w=S_w/e_2(p_1,\ldots,p_n)$ without ties, division
by $e_2(p_1,\ldots,p_n)^2$ gives the exact variance of $\widehat\tau_w$, and
under \eqref{eq:weight-diffuseness},
$\operatorname{Var}_0(\widehat\tau_w\mid p)\sim\frac{4}{9}s_{2,n}$.

\subsection{Exact conditional variance with ties for uniform masses}
